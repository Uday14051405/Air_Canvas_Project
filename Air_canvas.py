...
# Different array to handle different color points
bpoints = [deque(maxlen=1024)]
gpoints = [deque(maxlen=1024)]
rpoints = [deque(maxlen=1024)]
ypoints = [deque(maxlen=1024)]

# this color is used to identify that which color currently i am using
blue_index = 0
green_index = 0
red_index = 0
yellow_index = 0
...
# Now, code of white canvas setup 
paintWindow = np.zeros((471,636,3)) + 255      # here + 255 is for white color
# now drawing rectangle in white canvas
...
hands = mpHands.Hands(max_num_hands=1, min_detection_confidence=0.7)      # if you need two hands then do "max_num_hands=2"
#mpDraw = mp.solutions.drawing_utils         # it draws the landmark on the detected hands
...
while ret:
    # Read each frames from the webcam
    ret, frame = cap.read()
    ...
    if result.multi_hand_landmarks:
        ...
        for handslms in result.multi_hand_landmarks:
            ...
            fore_fingre = (landmarks[8][0], landmarks[8][1])
            center = fore_fingre
    ...
    # Now, printing the lines in both frames(on camera and white board)
